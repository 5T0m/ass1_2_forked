...
class CrossEncoder(nn.Module):
    ...
    def forward(self, pos_pairs, neg_pairs, grading=False):
        ...
        both the positive pairs and the negative pairs. For a given pair of (query, positive document) and (query, negative document), the model should "choose" the positive
        document. This can be done by setting the target label as the one matching the index of the positive document.
        Parameters
        ----------
        ...
        scores = torch.stack([pos_scores, neg_scores], dim=1).to(device)
        #scores = torch.cat([pos_scores, neg_scores], dim=0)
        labels = torch.zeros(scores.size(0), dtype=torch.long).to(device)
        # END SOLUTION
        if grading:
            return self.loss(scores, labels), {'pos_scores':pos_scores, 'neg_scores':neg_scores,
