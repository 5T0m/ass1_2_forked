...
class L1Regularizer(nn.Module):
    ...
    def forward(self, reps):
        ...
        """
        # BEGIN SOLUTION
        c_alpha = self.current_alpha
        l1_norms = torch.abs(reps).sum(dim=1)
        result = c_alpha * (torch.mean(l1_norms))
        return result
        # END SOLUTION
...
class SparseBiEncoder(nn.Module):
    ...
    def encode(self, input_ids, attention_mask, grading=False, **kwargs):
        ...
        **kwargs: 
        ...
        # Get logits from the output of the encoder model
        logits = output.logits
        
        # Zero-out the logits of all padded tokens
        logits_masked = logits.masked_fill(attention_mask.unsqueeze(-1) == 0, 0)
        
        # Apply ReLU and natural log transformation
        transformed_logits = torch.log(1 + torch.relu(logits_masked))
        
        # Perform max pooling over the second dimension
        sparse_reps, _ = torch.max(transformed_logits, dim=1)
        # END SOLUTION
        if grading:
    ...
    def score_pairs(self, queries, docs, return_regularizer=False, grading=False):
        ...
        # END SOLUTION
        if grading == True:
            return scores, query_reg, doc_reg, {'query_reps':query_reps, 'doc_reps':doc_reps}
    ...
    def forward(self, queries, pos_docs, neg_docs, grading=False):
        ...
        # END SOLUTION    
        if grading == True:
            return loss, pos_scores, neg_scores, {'query_reps': query_reps, 'pos_reps': pos_reps, 'neg_reps': neg_reps, 'query_reg': query_reg, 'pos_reg': pos_reg, 'neg_reg': neg_reg}
        else:
