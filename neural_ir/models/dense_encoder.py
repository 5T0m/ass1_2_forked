...
class DenseBiEncoder(nn.Module):
    ...
    def encode(self, input_ids, attention_mask, grading=False, **kwargs):
        ...
        The Dense Bi-Encoder uses the final hidden states as the representation of its input. To do so, an aggregation function is used to reduce the sequence of hidden states
        to a single vector. This is typically done by taking the mean or the max of the hidden states, or by exclusively using the representation of a special token like [CLS].
        For this assignment, we will use the mean of the hidden states, excluding the padding tokens
        Parameters
        ----------
        ...
        **kwargs: 
        ...
        last_hidden_state_masked = last_hidden_state.masked_fill(attention_mask.unsqueeze(-1) == 0, 0)

        # Sum the embeddings along the sequence dimension
        sum_embeddings = torch.sum(last_hidden_state_masked, dim=1)

        # Count the number of unpadded tokens in each sequence
        num_unpadded_tokens = torch.count_nonzero(attention_mask, dim=1)

        # Compute the mean of the embeddings
        reps = sum_embeddings / num_unpadded_tokens.unsqueeze(-1)
    ...
    def score_pairs(self, queries, docs):
        ...
        As the queries and documents are encoded separately, the score for a given query-document pair can be defined as either the dot product or the cosine similarity between
        the two vectors. For this assignment, we will use the dot product.
        Parameters
        ----------
        ...
        if torch.is_tensor(queries):
            ...
            #print("Not a tensor")
            queries = self.encode(queries.input_ids, queries.attention_mask)
        if torch.is_tensor(docs):
            pass
        else:
            docs = self.encode(docs.input_ids, docs.attention_mask)


        scores = torch.sum(queries * docs, dim=1)
    ...
    def forward(self, queries, pos_docs, neg_docs):
        ...
        """
        # BEGIN SOLUTION
        # Encode queries, positive documents, and negative documents
        q_vectors = self.encode(queries.input_ids, queries.attention_mask)
        pos_vectors = self.encode(pos_docs.input_ids, pos_docs.attention_mask)
        neg_vectors = self.encode(neg_docs.input_ids, neg_docs.attention_mask)
        
        # Compute scores for positive and negative document pairs
        pos_scores = self.score_pairs(q_vectors, pos_vectors)
        neg_scores = self.score_pairs(q_vectors, neg_vectors)
        
        # Compute the contrastive loss
        loss = torch.mean(torch.relu(neg_scores - pos_scores + 1))
